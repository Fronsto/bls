...
def sign_unique(params, sk, m):
	"""
	Sign messages while ensuring the messages will be unique 
	by appending the public key of the signer with the message.
	...
	Notes:
        To be used with aggregate_verify, where distinct messages are required
		to prevent rogue public key attacks. 
	"""
	assert len(m) > 0
	(G, o, g1, g2, e) = params
	vk = sk*g2 # sk*g2 gives the correspongind public key
	m.append(vk.export()) 
	digest = hash(m) 
	h = G.hashG1(digest)
	sigma = sk*h
...
def aggregate_verify(params, vks, sigma, m):
	"""
	Verfiy signatures for different messages, each associated with a separate public key.
	
	Parameters:
		- `params`: public parameters generated by `setup`
		- `vks` [G2Elem]: array containing verification keys
		- `sigma` (G1Elem): aggregate signature generated with `aggregate_sigma`
		- `m` [Bn]: array containing the messages
